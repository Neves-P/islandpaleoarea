...
for (time_slice in seq_along(archipelagos41_paleo)) {
  ...
  datalist <- datalist[which(!names(archipelagos41_paleo[[time_slice]]) %in% arch_to_remove)]

  for (archipelago in seq_along(datalist)) {
    area <- c(area, datalist[[archipelago]][[1]]$area)
    distance <- c(distance, datalist[[archipelago]][[1]]$distance_continent)
