...
replicate_number <- as.numeric(args[[1]])

replicate_number <- replicate_number

a_vals <- read.table("a_vals.txt", header = F)

the_dataset <- a_vals[replicate_number, 1]
the_init <- a_vals[replicate_number, 2]
model <- a_vals[replicate_number, 3]
...
to.write <- c(replicate_number, the_dataset, the_init, model, distance_dep, distance_type, as.matrix(lik_res), initparsopt, "\n")

file_name <- paste0("results_", replicate_number, ".txt")
